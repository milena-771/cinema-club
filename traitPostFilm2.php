        ...
        <main class="container">
          ...
          <div class="my-3 p-3 bg-body rounded shadow-sm">
            ...
            <?php
                ...
                echo "<div class=\"fw-bold\"> Où avez-vous vu <em>$_SESSION[titreFilm]</em> ?</div>";
                echo "<form action=\"traitPostFilm2.php\" method=\"post\">";
                echo "<div class=\"form-check-inline\">";
                echo "<input class=\"form-check-input\" type=\"radio\" name=\"lieuVisionnage\" id=\"auCine\" value=\"au cinéma\">";
                echo " <label class=\"form-check-label\" for=\"auCine\">au cinéma</label>";
                echo "  ";
                echo "<input class=\"form-check-input\" type=\"radio\" name=\"lieuVisionnage\" id=\"surPlateforme\" value=\"plateforme en ligne\">";
                echo " <label class=\"form-check-label\" for=\"surPlateforme\"> plateforme en ligne </label> ";
                echo "</div>";
                echo "<button class=\"btn btn-light\" type=\"submit\">Ajouter</button>";
                echo "</form>";

           /*<p> Où avez-vous vu <?php echo $_SESSION[titreFilm]; ?> ?</p>
           <form action="traitPostFilm2.php" method="post">
             <div class="form-check-inline">
               <input class="form-check-input" type="radio" name="lieuVisionnage" id="auCine" value="au cinéma">
               <label class="form-check-label" for="auCine">au cinéma</label>
               <input class="form-check-input" type="radio" name="lieuVisionnage" id="surPlateforme" value="plateforme en ligne">
               <label class="form-check-label" for="surPlateforme">plateforme en ligne</label>
            </div>
               <button class="btn btn-light" type="submit">Ajouter</button>
           </form>*/


          //if(isset($_GET[AjouterList])){


             if(isset($_POST[lieuVisionnage]) && !empty($_POST[lieuVisionnage]) ){

               //header ("Location: http://localhost:8888/app_cine/ajoutListFilm2.php");
               $titreFilm = $_SESSION[titreFilm];
               $idFicheFilm = $_SESSION[idFicheFilm];
               $idUtilisateur = $_SESSION[idUtilisateur];
               $realFilm = $_SESSION[real_film];
               $dateVu = date('Y-m-d');
               $monCinema = $_POST[lieuVisionnage];
               $idPostFilm = 000;
               ...
               if($line = mysqli_fetch_assoc($resultSQL)){
                 ...
                 echo "<div class=\"border-bottom fw-bold\">Mes films vus en 2022 :</div>";
